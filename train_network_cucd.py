...
from utils.evaluation import model_evaluation, model_testing_cucd
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch in save_checkpoints and not cfg.DEBUG:
            ...
            if cfg.DATASETS.TESTING is not None:
                model_testing_cucd(net, cfg, device, global_step, epoch_float)
...
if __name__ == '__main__':
    ...
    if not cfg.DEBUG:
        wandb.init(
            name=cfg.NAME,
            project='urban_extraction_cucd',
            tags=['run', 'urban', 'extraction', 'segmentation', 'cucd', ],
        )
